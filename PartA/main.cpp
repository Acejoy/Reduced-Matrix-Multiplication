...
{ 
  // // creating perf_event arrays for counting performance counters
  // int m=5;
  ...
  // // initialising values to perf_event struct
  // for(int i=0; i<m; i++){
  //     memset(&pArr[i].pea, 0, sizeof(struct perf_event_attr));     
  // }

  // // defining the event for each counter
  ...
  // for(int i=0; i<m; i++){
  //     ioctl(pArr[i].fd, PERF_EVENT_IOC_RESET, 0); // resets the counter to 0
  //     ioctl(pArr[i].fd, PERF_EVENT_IOC_ENABLE, 0); // enables for reading the count.
  // }
